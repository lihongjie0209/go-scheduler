...
ALTER TABLE jobs ADD CONSTRAINT jobs_script_definition_check CHECK (
    ...
    (script_language IN ('shell','python','nodejs','php','powershell') AND script_source <> '' AND executor_group_id IS NOT NULL AND executor_handler = '__script__')
);
...
ALTER TABLE job_script_versions ADD CONSTRAINT job_script_versions_script_language_check
    CHECK (script_language IN ('shell','python','nodejs','php','powershell'));
