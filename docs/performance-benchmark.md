...
后续 `pg_stat_statements` 诊断确认，到期入队存在按任务查询路由策略、查询队列状态、插入运行和更新时间的 N+1。普通 `parallel` 任务不需要读取活跃队列状态，因此改为一次批量读取路由策略，并通过 pgx batch 集中发送运行插入和任务时间推进。单条 Executor 日志使用直接 INSERT 路径，正常执行器候选查询同时应用标签条件，进一步减少 SQL 往返。

`ClaimRuns` 原先分别扫描 `job_runs` 生成 job 和 tenant 两份活跃计数。改为物化一次活跃集合，再分别聚合，保持并发限制语义不变。1,000 个同时到期任务、64 workers 的三轮结果：

| 轮次 | 吞吐 | P50 | P99 |
| --- | ---: | ---: | ---: |
| 1 | 524.90/s | 1,024 ms | 1,828 ms |
| 2 | 390.06/s | 1,630 ms | 2,556 ms |
| 3 | 435.17/s | 1,473 ms | 2,289 ms |
| 中位数 | 435.17/s | 1,473 ms | 2,289 ms |

三轮均为 1,000/1,000 成功且零丢失、零重复。相对本轮 Claim 改写前的三轮中位数 415.38/s、P99 2,396 ms，吞吐提高 4.8%，P99 降低 4.5%。相对最初 64 workers gRPC 基线 276.11/s、P99 3,617 ms，当前中位吞吐累计提高 57.6%，P99 降低 36.7%。

### Claim 低水位批量补充

每个 worker 完成后立即唤醒调度器会产生大量小批次 Claim。64 workers、1,000 个任务的统计轮次中，`ClaimRuns` 被调用 282 次。改为累计至少四分之一 worker 空槽后批量补充；所有 worker 空闲时仍立即唤醒，周期 tick 继续承担兜底。

优化后的 `pg_stat_statements` 统计轮次中，Claim 调用降至 83 次，SQL 总执行时间从约 818 ms 降至 169 ms。三轮端到端结果：

| 轮次 | 吞吐 | P50 | P99 |
| --- | ---: | ---: | ---: |
| 1 | 552.87/s | 900 ms | 1,805 ms |
| 2 | 395.54/s | 1,757 ms | 2,525 ms |
| 3 | 514.27/s | 1,086 ms | 1,943 ms |
