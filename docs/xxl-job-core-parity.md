...
| 通知 | 生命周期 Webhook、邮件、钉钉 | 每个订阅可选择 pending/running/waiting_callback/succeeded/failed/timed_out/cancelled/skipped/exhausted 事件，并绑定全部或指定任务；webhook/email/dingtalk 按“事件×渠道”独立投递，PG SKIP LOCKED 多 Core 抢占；每渠道可配置重试次数、指数退避上下限，耗尽进入 dead；支持版本化更新、启停和保留历史的软删除；钉钉支持机器人地址、access token/HMAC 和模板 | `notifications create/update/list/enable/disable/delete/history` | 已覆盖且扩展 | U: 事件/钉钉配置、模板、HMAC、退避、CLI 生命周期与历史游标；M: migration 24–26、任务/租户过滤、租约 fencing、死信、游标历史、版本冲突与软删除；X: gRPC 渠道完整生命周期；E: CLI 配置双 webhook，最终失败后健康渠道 1 次、暂时失败渠道重试 |
