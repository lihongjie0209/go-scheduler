...
| 通知 | 生命周期 Webhook、邮件、钉钉 | 每个订阅可选择 pending/running/waiting_callback/succeeded/failed/timed_out/cancelled/skipped/exhausted 事件，并绑定全部或指定任务；webhook/email/dingtalk 按“事件×渠道”独立投递，PG SKIP LOCKED 多 Core 抢占；每渠道可配置重试次数、指数退避上下限，耗尽进入 dead；钉钉支持机器人地址、access token/HMAC 和模板 | `notifications create/list/history` | 已覆盖且扩展 | U: 事件/钉钉配置、模板、HMAC、退避和 CLI 历史过滤；M: migration 24、任务过滤、死信和历史查询；X: gRPC 渠道创建/历史查询；E: CLI 配置双 webhook，最终失败后健康渠道 1 次、暂时失败渠道重试 |
