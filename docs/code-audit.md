...
- Executor 在确认 Dispatch 前先写节点本地 execution inbox，完成回报再写 durable outbox 后异步发送给 Core；重启后可接管 Docker/Kubernetes 外部执行，进程内任务明确失败并进入 Core 重试策略。状态目录可能包含任务输入、callback token、仓库及 Kubernetes 凭据，部署必须使用每节点独占的加密或受限持久卷。
